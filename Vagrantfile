...
$controller_vm_memory = 2560
$worker_vm_memory = 4096

puts 'Workers need at least 1536 MB of memory' if $worker_vm_memory < 1536
...
def signTLS(is_ca:, subject:, issuer_subject: '', issuer_cert: nil, public_key:, ca_private_key:, key_usage: '', extended_key_usage: '', san: '')
  ...
  cert.issuer = if is_ca
                  OpenSSL::X509::Name.parse(subject)
                else
                  OpenSSL::X509::Name.parse(issuer_subject)
                end
  cert.not_before = Time.now
  cert.not_after = Time.now + 365 * 24 * 60 * 60
  cert.public_key = public_key
  cert.serial = Random.rand(1..65_534)
  cert.version = 2

  ef = OpenSSL::X509::ExtensionFactory.new
  ef.subject_certificate = cert
  ef.issuer_certificate = if is_ca
                            cert
                          else
                            issuer_cert
                          end
...
Vagrant.configure('2') do |config|
  ...
  FileUtils.rm_rf('provisioning/etc') if ARGV[0] == 'destroy'
  ...
  config.vm.define vm_name = 'w1' do |worker|
    ...
    worker.vm.provider :virtualbox do |_vb|
  ...
  config.vm.define vm_name = 'c1' do |controller|
    ...
    controller.vm.provider :virtualbox do |_vb|
